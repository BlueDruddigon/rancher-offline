...
from base.triton_infer import PredictTriton5point, PredictTriton5pointv2
...
class FaceDetectorv2:
    def __init__(self, rac, param_file=None, triton=True, detector=PredictTriton5pointv2()):
        self.param_file = param_file
        self.rac = rac # net3l
        self.default_image_size = (480, 640)
        self.triton5point = detector
        self.triton = triton

    def prepare(self, ctx_id=0, nms=0.4, fix_image_size=None):
        if not self.triton:
    ...
    def detect(self, img, threshold=0.62, scale=1.0):
        proposals_list = []
        scores_list = []
        landmarks_list = []
        if scale==1.0:
        ...
        input_blob = np.array(im_tensor, dtype=np.float32)

        if not self.triton:
            data = nd.array(im_tensor)
            # np.save('/mnt/NhatNT/f19-ai-cv-face-search/align/input_mnet25', data.asnumpy())
            ...
            net_out = self.triton5point.predict([input_blob])
            # print("time 5 point: ", time.time() - t)
            # print("net_out: ", len(net_out))
...
class FaceDetector(FaceDetectorv2):
    def __init__(self):
        super().__init__(rac='net3', param_file=None, triton=True, detector=PredictTriton5point())
